...
// Prefer lookup by referral code to avoid exposing numeric order IDs in the URL
$ref = isset($_GET['ref']) ? trim($_GET['ref']) : '';
$order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
$order = null;
if ($ref !== '') {
    $sql = "SELECT * FROM orders WHERE referral_code = '" . $conn->real_escape_string($ref) . "' LIMIT 1";
    $res = $conn->query($sql);
    if ($res && $res->num_rows) {
...
// compute expected delivery window (24 - 72 hours after order time). Fallback to now
$expected_from = '';
$expected_to = '';
$order_ts = time();
...
$expected_from = date('j M Y, H:i', $order_ts + 24 * 3600);
$expected_to   = date('j M Y, H:i', $order_ts + 72 * 3600);
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <div class="container">
            <div class="thank-you-card">
                ...
                <div class="order-details">
                    <h3>Order Details</h3>
                    <!-- Order ID hidden for customer privacy -->
                    <div class="detail-row">
                        <span class="detail-label">Customer:</span>
                        <span class="detail-value" id="customer-name"><?php echo $order ? esc($order['fullname']) : 'Jane Doe'; ?></span>
                    </div>
                    <div class="detail-row">
                        <span class="detail-label">Address:</span>
                        <span class="detail-value" id="customer-address"><?php echo $order ? esc($order['address']) : '123 Example St, Lagos'; ?></span>
                    </div>
                    <div class="detail-row">
                    ...
                    <div class="detail-row">
                        <span class="detail-label">Expected Delivery (24–72 hrs):</span>
                        <span class="detail-value" id="expected-delivery"><?php echo esc($expected_from . ' — ' . $expected_to); ?></span>
                    </div>
                </div>
                ...
                <div class="whatsapp-section">
                    ...
                    <div class="whatsapp-buttons">
                        ...
                        <a href="<?php echo $order ? 'https://wa.me/2348102609396?text='.urlencode('Hello Emerald Tech Hub! I would like to track my recent order.\n\nPackage: '.($order['pack']).'\nName: '.($order['fullname'])).'' : '#'; ?>" class="whatsapp-button" id="whatsapp-track">
                            <i class="fab fa-whatsapp"></i>
